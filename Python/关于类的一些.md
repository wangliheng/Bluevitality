...
class student(object):
 ...
    count='string'                      #类属性（实例共有，静态成员）若每个对象都具有相同字段则可以用'静态字段'
					#普通字段的内容保存对象中，即：根据此类创建了多少对象，在内存中就有多少个普通字段。
...
#方法包括：普通方法、静态方法、类方法，三种方法在内存中都归属于类!! 区别在于调用方式的不同：
 ...
    def __init__(self,name,age):        #构造函数，创建实例时即进行初始化操作
        self.__name=name                #定义了两个不可以被直接访问的实例变量
        self.__age=age

    def __speak(self,word='default'): 	#定义了私有化函数，它不可被直接访问也不可以被子类继承
 ...
        if hi is None:			#它不会对任何实例类型进行操作（静态方法不能访问类变量和实例变量）
            hi = 'hello'
        print(hi)
