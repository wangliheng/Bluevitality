####
```bash
#此文不介绍Linux内核的编译及编译配置，只讲述在编译完成之后的内核部署相关内容，此文环境 CentOS 7，内核源码版本 4.4.12 。
#注意编译安装完成后 /usr/src/linux-4.4.12 目录不能删除
...
#将会从 /lib/modules/4.4.12 目录生成 initramfs-4.4.12.x86_64.img 文件，便是临时文件系统，可以看到其下的文件目录
➜  4.4.12 ls -al
